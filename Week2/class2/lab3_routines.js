...
const transferPaymentForProvincePromise = (gocData, provCode) => {
    return new Promise((resolve) => {
        provinces.forEach(province => {
            if (province.code === provCode) {

            }
            let data = console.log(`${province.name} had a transfer payment of ${currencyFormatter(
                gocData[`${province.code.toLowerCase()}`][`${FISCALYEAR}`]
            )}`)
            resolve(data);
        })
    })
