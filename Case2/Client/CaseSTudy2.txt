...
import {
    ...
    CardContent,
    Typography,
    TextField, Avatar, Dialog, DialogTitle, DialogContent, ListItem, RadioGroup, FormControlLabel, Radio
} from "@mui/material";
...
import UserList from "./usermessagelist";
const Scenario1Test = () => {
    const initialState = {
        chatName: "",
        roomName: "",
    ...
    return (
        <ThemeProvider theme={theme}>
            <TopBar showjoinfields={state.showjoinfields} viewDialog={handleOpenDialog} />
            <Dialog open={open} onClose={handleCloseDialog} style={{ margin: 20 }}>
                ...
                <DialogContent>
                    ...
                    <List>
                        {state.users.filter((user) => user.chatName !== "Admin")
                            .map((user, index) => (
                                <ListItem key={index}>
                                    <ListItemIcon>
                                    ...
                                    <Typography>{user.chatName} in room {user.roomName}</Typography>
                                </ListItem>
                            ))}
                    </List>
                </DialogContent>
                ...
                <CardContent>
                    ...
                    {state.showjoinfields ? (
                        <div>
                            <TextField
                                ...
                                error={state.status !== ""}
                                helperText={state.chatName === "" ? "Enter the Chat Name" : state.status}
                            ...
                            <TextField
                                ...
                                helperText={state.roomName === "" ? "Enter the room name" : null}
                                sx={{ '& .MuiFormHelperText-root': { color: 'red' } }}
                            />
                    ...
                    {!state.showjoinfields ? (
                        ...
                        <div>
                            ...
                            <UserList messages={state.messages} chatName={state.chatName} roomName={state.roomName} />
